...
export const actions = {
  async nuxtServerInit({ dispatch }, { app }) {
    await dispatch('fetchSiteData', app.store.state.i18n.locale || 'fr');
  },
  async onLocaleChanged({ dispatch }, newLocale) {
    // Perform actions similar to nuxtServerInit here, based on newLocale
    // Fetch data specific to the new locale, etc.
    console.log('Locale changed to:', newLocale);
    await dispatch('fetchSiteData', newLocale);
  },
  async fetchSiteData({ commit }, locale) {
    const app = this;
